...
class tests(unittest.TestCase):
    ...
    def test_ppf(self):
        ...
        self.assertAlmostEqual(
            ttt.ppf(0.3, ttt.N01.mu, ttt.N01.sigma), -0.5244005127080409
        )
        N23 = ttt.Gaussian(2.0, 3.0)
    ...
    def test_compute_margin(self):
        ...
        self.assertAlmostEqual(
            ttt.compute_margin(0.25, math.sqrt(2) * 25.0 / 6), 1.8776004584348176
        )
        self.assertAlmostEqual(
            ttt.compute_margin(0.25, math.sqrt(3) * 25.0 / 6), 2.2995815319905395
        )
        self.assertAlmostEqual(ttt.compute_margin(0.0, math.sqrt(3) * 25.0 / 6), 0.0)
        self.assertAlmostEqual(
            ttt.compute_margin(1.0, math.sqrt(3) * 25.0 / 6), math.inf
        )

    def test_trunc(self):
        """Test truncated Gaussian distribution parameters for win/draw outcomes."""
        mu, sigma = ttt.trunc(*ttt.Gaussian(0, 1), 0.0, False)
        self.assertAlmostEqual(mu, 0.7978845368663289)
        self.assertAlmostEqual(sigma, 0.6028103066716792)
        mu, sigma = ttt.trunc(
            *ttt.Gaussian(0.0, math.sqrt(2) * (25 / 6)), 1.8776005988, True
        )
        self.assertAlmostEqual(mu, 0.0)
        self.assertAlmostEqual(sigma, 1.0767055, places=4)
        mu, sigma = ttt.trunc(
            *ttt.Gaussian(12.0, math.sqrt(2) * (25 / 6)), 1.8776005988, True
        )
    ...
    def test_1vs1(self):
        ...
        g = ttt.Game(
            [
                [ttt.Player(ttt.Gaussian(29.0, 1.0), 25.0 / 6)],
    ...
    def test_1vs1_draw(self):
        ...
        ta = [ttt.Player(ttt.Gaussian(25.0, 3.0), 25.0 / 6, 25.0 / 300)]
        tb = [ttt.Player(ttt.Gaussian(29.0, 2.0), 25.0 / 6, 25.0 / 300)]
    ...
    def test_1vs1vs1_draw(self):
        ...
        ta = [ttt.Player(ttt.Gaussian(25.0, 3.0), 25.0 / 6, 25.0 / 300)]
        tb = [ttt.Player(ttt.Gaussian(25.0, 3.0), 25.0 / 6, 25.0 / 300)]
        tc = [ttt.Player(ttt.Gaussian(29.0, 2.0), 25.0 / 6, 25.0 / 300)]
    ...
    def test_NvsN_Draw(self):
        ...
        ta = [
            ttt.Player(ttt.Gaussian(15.0, 1.0), 25.0 / 6, 25.0 / 300),
            ttt.Player(ttt.Gaussian(15.0, 1.0), 25.0 / 6, 25.0 / 300),
        ]
        tb = [ttt.Player(ttt.Gaussian(30.0, 2.0), 25.0 / 6, 25.0 / 300)]
    ...
    def test_NvsNvsN_mixt(self):
        ...
        ta = [
            ttt.Player(ttt.Gaussian(12.0, 3.0), 25.0 / 6, 25.0 / 300),
            ttt.Player(ttt.Gaussian(18.0, 3.0), 25.0 / 6, 25.0 / 300),
        ]
        tb = [ttt.Player(ttt.Gaussian(30.0, 3.0), 25.0 / 6, 25.0 / 300)]
        tc = [
            ttt.Player(ttt.Gaussian(14.0, 3.0), 25.0 / 6, 25.0 / 300),
            ttt.Player(ttt.Gaussian(16.0, 3.0), 25.0 / 6, 25.0 / 300),
        ]
    ...
    def test_evidence_1vs1(self):
        ...
        ta = [ttt.Player(ttt.Gaussian(25.0, 1e-7), 25.0 / 6, 25.0 / 300)]
        tb = [ttt.Player(ttt.Gaussian(25.0, 1e-7), 25.0 / 6, 25.0 / 300)]
    ...
    def test_1vs1vs1_margin_0(self):
        ...
        ta = [ttt.Player(ttt.Gaussian(25.0, 1e-7), 25.0 / 6, 25.0 / 300)]
        tb = [ttt.Player(ttt.Gaussian(25.0, 1e-7), 25.0 / 6, 25.0 / 300)]
        tc = [ttt.Player(ttt.Gaussian(25.0, 1e-7), 25.0 / 6, 25.0 / 300)]

        proba = 0
        proba += ttt.Game([ta, tb, tc], [3, 2, 1], 0.0).evidence
        proba += ttt.Game([ta, tb, tc], [3, 1, 2], 0.0).evidence
        proba += ttt.Game([ta, tb, tc], [2, 3, 1], 0.0).evidence
        proba += ttt.Game([ta, tb, tc], [1, 3, 2], 0.0).evidence
        proba += ttt.Game([ta, tb, tc], [2, 1, 3], 0.0).evidence
        proba += ttt.Game([ta, tb, tc], [1, 2, 3], 0.0).evidence
    ...
    def test_12_factorial_combinations(self):
        ...
        evidence = ttt.Game(teams, list(range(12)), 0.0).evidence
    ...
    def test_history_init(self):
        ...
        for k in ["aa", "b", "c"]:
            priors[k] = ttt.Player(
                ttt.Gaussian(25.0, 25.0 / 3), 25.0 / 6, 0.15 * 25.0 / 3
            )
    ...
    def test_one_batch_history(self):
        ...
        priors = dict()
        for k in ["aj", "bj", "cj"]:
            priors[k] = ttt.Player(ttt.Gaussian(25.0, 25.0 / 3), 25.0 / 6, 25.0 / 300)
    ...
    def test_trueSkill_Through_Time(self):
        ...
        results = [[1, 0], [0, 1], [1, 0]]
        priors = {
            k: ttt.Player(ttt.Gaussian(25.0, 25.0 / 3), 25.0 / 6, 25.0 / 300)
            for k in ["a", "b", "c"]
        }
    ...
    def test_env_0_TTT(self):
        ...
        h = ttt.History(
            composition=composition, mu=0.0, sigma=6.0, beta=1.0, gamma=0.05
        )
    ...
    def test_teams(self):
        ...
        # print(h.learning_curves())
        self.assertAlmostEqual(
            h.bskills[0]["a"].posterior.mu, h.bskills[0]["b"].posterior.mu, 5
        )
        self.assertAlmostEqual(
            h.bskills[0]["a"].posterior.sigma, h.bskills[0]["b"].posterior.sigma, 5
        )
        self.assertAlmostEqual(
            h.bskills[0]["c"].posterior.mu, h.bskills[0]["d"].posterior.mu, 3
        )
        self.assertAlmostEqual(
            h.bskills[0]["c"].posterior.sigma, h.bskills[0]["d"].posterior.sigma, 3
        )
        self.assertAlmostEqual(
            h.bskills[1]["e"].posterior.mu, h.bskills[1]["f"].posterior.mu, 3
        )
        self.assertAlmostEqual(
            h.bskills[1]["e"].posterior.sigma, h.bskills[1]["f"].posterior.sigma, 3
        )
    ...
    def test_sigma_beta_0(self):
        ...
        for k in ["a_b", "c_d", "e_f", "b_c", "a_d", "e_f"]:
            priors[k] = ttt.Player(
                ttt.Gaussian(mu=0.0, sigma=1e-7), beta=0.0, gamma=0.2
            )
        h = ttt.History(
            composition=composition,
            results=results,
    ...
    def test_1vs1_with_weights(self):
        ...
        post = g.posteriors()
        self.assertTrue(
            post[0][0].isapprox(ttt.Gaussian(5.557176746, 4.0527906913), 1e-3)
        )
    ...
    def test_NvsN_with_weights(self):
        ta = [
            ttt.Player(ttt.Gaussian(25.0, 25.0 / 3), 25.0 / 6, 0.0),
            ttt.Player(ttt.Gaussian(25.0, 25.0 / 3), 25.0 / 6, 0.0),
        ]
        wa = [0.4, 0.8]
        tb = [
    ...
    def test_1vs1_TTT_with_weights(self):
        ...
        h = ttt.History(
            composition, mu=2.0, beta=1.0, sigma=6.0, gamma=0.0, weights=weights
        )
        h.forward_propagation()
        lc = h.learning_curves()
        self.assertTrue(
            lc["a"][0][1].isapprox(ttt.Gaussian(5.53765944, 4.758722), 1e-4)
        )
        self.assertTrue(
            lc["b"][0][1].isapprox(ttt.Gaussian(-0.83012755, 5.2395689), 1e-4)
        )
        self.assertTrue(
            lc["a"][1][1].isapprox(ttt.Gaussian(1.7922776, 4.099566689), 1e-4)
        )
        self.assertTrue(
            lc["b"][1][1].isapprox(ttt.Gaussian(4.8455331752, 3.7476161), 1e-4)
        )
        ...
        self.assertTrue(
            lc["a"][0][1].isapprox(
                ttt.Gaussian(lc["a"][0][1].mu, lc["a"][0][1].sigma), 1e-4
            )
        )
        self.assertTrue(
            lc["b"][0][1].isapprox(
    ...
    def test_fixed_point_approx(self):
        ...
        self.assertAlmostEqual(
            ttt.fixed_point_approx(r, mu, sigma),
