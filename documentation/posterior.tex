...
\todo[inline]{Notar que el paper original aplica la convergencia TTT solo al interior de un a\~no: ``we go through the game outcomes within a year t several times until convergence''.}
...
\paragraph{Forward - First update} The update for a game outcome is performed in the same way as original TrueSkill, but saving (A) the upward messages $m_{f_p\rightarrow s}(s)$, i.e. the variational likelihood, and (C) the forward messages $m_{f_{s^{t+1}}\rightarrow s^{t+1}}(s^{t+1})$, i.e. the variational posterior.
