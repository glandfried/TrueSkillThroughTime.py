...
\paragraph{Forward - First update} The update for a game outcome is performed in the same way as original TrueSkill, but saving the upward messages $m_{f_p\rightarrow s}(s)$, i.e. the variational likelihood, and the forward messages $m_{f_{s^{t+1}}\rightarrow s^{t+1}}(s^{t+1})$, i.e. the variational posterior.

\paragraph{Backward} In the backward pass, the backward message $m_{{f_{s^t}}\rightarrow s^{t-1}}(s^{t-1})$ serve as the new prior for time step $t-1$.
...
\begin{equation}
 \overbrace{m_{f_{s^t}\rightarrow s^{t-1}}(s^{t-1})}^{\text{New prior $t-1$}} = \int f_{s^t} \overbrace{m_{f_{p^t}\rightarrow s^t}(s^t)}^{\text{Last likelihood}}
\end{equation}
