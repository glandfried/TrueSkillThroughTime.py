...
m_{d \rightarrow f_r}(d) & \overset{\hfrac{\text{eq}}{\ref{eq:m_v_f}}}{=} \prod_{g \in n(t_e) \setminus  \{f_{d_{k}} \}} m_{g \rightarrow t_e} (t_e) \\[0.3cm] 
 & \overset{\hfrac{\text{fig}}{\ref{modelo_trueskill_2vs2}}}{=} m_{f_d \rightarrow d}(d) \overset{\hfrac{\text{eq}}{\ref{eq:m_fd_d}}}{=} N(d_1 | \delta_1, \vartheta_1)
\end{split}
\end{equation}
...
\begin{split}
 ...
 & \overset{\hfrac{\text{fig}}{\ref{modelo_trueskill_2vs2}}}{\underset{\hfrac{\text{eq}}{\ref{eq:m_d_fr}}}{=}} \int \mathbb{I}(d_1 > 0) N(d_1 | \delta_1, \vartheta_1)  dd_1 \\[0.2cm]
 &  \overset{\hfrac{\text{eq}}{\ref{eq:estandarizar}}}{=} \int \mathbb{I}(d_1 > 0) N( \frac{d_1 - \delta_1}{\vartheta_1}) dd_1 \\[0.2cm]
 & = 1 - \Phi(\frac{-\delta_1}{\vartheta_1}) \\
 & = \Phi(\frac{\delta_1}{\vartheta_1})
\end{split}
\end{equation}
...
\begin{equation}\label{eq:p*_d}
 \widehat{P}(d_1) \overset{\hfrac{\text{eq}}{\ref{eq:mean_aprox_}}}{\underset{\hfrac{\text{eq}}{\ref{eq:variance_aprox_}}}{=}} N\Bigg(d1 \,  \bigg| \, \underbrace{ \delta_1 + \vartheta_1 \, V_1(t) \,}_{\hfrac{\text{\scriptsize Media}}{\text{\scriptsize aproximada}} \, \text{\small $\widehat{\delta}_1$}} , \,  \underbrace{ \vartheta_1^2 \big( 1 - W_1(t) \big) }_{\hfrac{\text{\scriptsize Varianza}}{\text{\scriptsize aproximada}} \,\text{\small $\widehat{\vartheta}^{\,2}_1$}} \, \Bigg)
\end{equation}
...
\begin{split}
 ...
 & \overset{\hfrac{\text{eq}}{\ref{eq:p*_d}}}{\underset{\hfrac{\text{eq}}{\ref{eq:m_fd_d}}}{=}}  \frac{N(d_1 \,  | \,\widehat{\delta}_1 , \, \widehat{\vartheta}^{\,2}_1 )}{N(d_1 | \delta_1, \vartheta^2_1)} \\
 &\overset{\hfrac{\text{sec}}{\ref{sec:division_normales}}}{\propto} N(d_1,\delta_{\div},\vartheta_{\div}^2 )
\end{split}
\end{equation}

\begin{equation}
 \vartheta_{\div} = \sqrt{\frac{\widehat{\vartheta}^{\,2}_1\vartheta^2_1}{(\vartheta^2_1 - \widehat{\vartheta}^{\,2}_1)}}
\end{equation}

\begin{equation}
 \delta_{\div} = \frac{(\vartheta^2_1\widehat{\delta}_1 - \widehat{\vartheta}^{\,2}_1\delta_1)}{(\vartheta^2_1 - \widehat{\vartheta}^{\,2}_1)}
\end{equation}
...
The factor graph neighbor to a within skill variable $s_i^t$, is

\begin{center}
\tikz{ %        
        \node[latent] (s0) {$s_i^{t-1}$} ; %
        
        \node[factor, right=of s0 ] (fs1) {} ;
        \node[const, above=of fs1] (nfs1) {$f_{s_i^{t}}$}; %
        
        \node[latent, right=of fs1, xshift=1cm] (s1) {$s_i^t$} ; %
        
        \node[factor, right=of s1, xshift=1cm ] (fs2) {} ;
        \node[const, above=of fs2] (nfs2) {$f_{s_i^{t+1}}$}; %
        
        \node[latent, right=of fs2] (s2) {$s_i^{t+1}$} ; %
        
        \node[factor, below=of s1,xshift=-2cm] (fp0) {} ;
        \node[const, right=of fp0] (nfp0) {$f_{p_i^{t}(1)}$}; %
        
        \node[factor, below=of s1] (fp1) {} ;
        \node[const, right=of fp1] (nfp1) {$f_{p_i^{t}(2)}$}; %
        
        \node[factor, below=of s1,xshift=2cm] (fp2) {} ;
        \node[const, right=of fp2] (nfp2) {$f_{p_i^{t}(k)}$}; %
...
By the sum-product algorithm we know that the marginal distribution of a variable $s$ is the product of the messages that their neighbors $n(s)$ send to him

\begin{equation}
 p(s_i^t) = \prod_{h \in n(s_i^t)} m_{h \rightarrow s_i^t}(s_i^t)
\end{equation}

The basic idea is to update repeatedly on the same game outcomes but making sure that the effect of the previous update on that game outcome is removed before the new effect is added.

\vspace{0.3cm}

For a player $i$ we will call:
\begin{itemize}
 \item \textbf{\emph{posterior}$\bm{(t-1)}$: } $m_{s^{t-1}\rightarrow f_{s^t}}(s^{t-1}) = p(s^{t-1}) \cdot m_{f_{s^t} \rightarrow s^{t-1}}(s^{t-1})^{-1}$
 \item \textbf{\emph{prior}$\bm{(t)}$: } $m_{f_{s^t} \rightarrow s^t}(s^t) = \int f_{s^t} \cdot \text{\emph{posterior}}(t-1) \, ds^{t-1}$
 \item \textbf{\emph{likelihood}$\bm{(t,k)}$: } $m_{f_{p^{t}(k)} \rightarrow s^t}(s^t)$
 \item \textbf{withinPrior(t,k): } $m_{s^t \rightarrow f_{p^t(k)}}(s^t) =  p(s^t) \cdot m_{f_{p^t(k)} \rightarrow s^t}(s^t)^{-1}$
 \item \textbf{inversePrior (t): } $m_{f_{s^{t+1}} \rightarrow s^t}(s^t) = \int f_{s^t} \cdot \underbrace{p(s^{t+1}) \cdot m_{f_{s^{t+1}} \rightarrow s^{t+1}}(s^{t+1})^{-1}}_{\text{inversePosterior (t+1)}} ds^{t+1}$
 \item \textbf{\emph{inversePosterior}$\bm{(t+1)}$: }
\end{itemize}

Then we can express the marginal distribution as,  

\begin{equation}
 p(s^t) =  \emph{prior}(t)\emph{inversPrior}(t) \prod_{k=1}^K^t 
\end{equation}


When a messages is not yet defined, like the inverse prior in the first forward pass, you can avoid it, or you can replace it by a unit scalar or by Gaussian distribution with infinite variance.



\vspace{0.3cm}

Within a temporal batch $t$, we go through the games outcomes $\vm{y}^t$ several times until convergence.
The update for a game outcome $y^t_i(k)$ is performed by the same way as original TrueSkill but saving the upward message $s_i^t$, $m_{f_{p_i^t(k)} \rightarrow s_i^t}(s_i^t)$.
So when a game outcome comes up for update again, the new downward message can be computed as, 
\begin{equation}
 m_{f_{p_i^t(k)} \rightarrow s_i^t}(p_i^t(k)) = \int f_{p_i^t(k)} \cdot \underbrace{p(s_i^t) \cdot m_{f_{p_i^t(k)} \rightarrow s_i^t}(s_i^t)^{-1}}_{\text{Within prior}} ds_i^t
\end{equation}

\vspace{0.3cm}

The first forward pass is identical to the inference pass of the original TrueSkill, except that the posteriors $m_{f_{s^{t+1}} \rightarrow s^{t+1}}(s^{t+1})$ are stored.
...
\paragraph{Backward} In the backward pass (D), the backward message $m_{{f_{s^t}}\rightarrow s^{t-1}}(s^{t-1})$ serve as the new prior for time step $t-1$.

\begin{verbatim}
(s^t-1)<=<=[f_s^t]----(s^t)<-<-[f_s^t+1]----(s^t+1) 
   |                    ^                      |
                        ^
\end{verbatim} 

By the rules of the sum-product algorithm this message is,

\begin{equation}
\begin{split}
 \overbrace{m_{f_{s^t}\rightarrow s^{t-1}}(s^{t-1})}^{\text{Back prior $t-1$}} &= \int f_{s^t} \overbrace{m_{s^t \rightarrow f_s^t}(s^t)}^{\text{Inverse posterior $t$}} \\
 & = \int f_{s^t} \underbrace{m_{f_{p^t}\rightarrow s^t}(s^t)}_{\text{Last likelihood $t$}} \underbrace{m_{f_{s^{t+1}}\rightarrow s^t}(s^t)}_{\text{Back prior $t$ }}
\end{split}
\end{equation}

When $t$ is an endpoint then this message it is simplified as,

\begin{equation}
 \overbrace{m_{f_{s^t}\rightarrow s^{t-1}}(s^{t-1})}^{\text{Back prior $t-1$}} = \int f_{s^t} \overbrace{m_{f_{p^t}\rightarrow s^t}(s^t)}^{\text{Last likelihood $t$}}
\end{equation}
% 
% The endpoint back message will be,
% 
% \begin{equation}
% \begin{split}
% \overbrace{m_{f_{s^t}\rightarrow s^{t-1}}(s^{t-1})}^{\text{Back prior $t-1$}} &= \int \overbrace{\N(s^{t}|s^{t-1},\tau^2)}^{f_{s^t}} \overbrace{\N(s^t|\mu_{\text{last like}^t},\sigma_{\text{last like}^t}^2)}^{\text{Last likelihood $t$}} ds^t \\
% & \propto \N(s^{t-1}|\mu_{\text{last like}^t},\sigma_{\text{last like}^t}^2 + \tau^2)
% \end{split}
% \end{equation}
% 
% The non endpoint back message will be,
% 
% \begin{equation}
% \begin{split}
% \overbrace{m_{f_{s^t}\rightarrow s^{t-1}}(s^{t-1})}^{\text{Back prior $t-1$}} &= \int \overbrace{\N(s^{t}|s^{t-1},\tau^2)}^{f_{s^t}} \overbrace{\N(s^t|\mu_{\text{like}^t},\sigma_{\text{like}^t}^2)}^{\text{Last likelihood $t$}}  \overbrace{m_{f_{s^{t+1}}\rightarrow s^t}(s^t)}^{\text{Back prior $t$ }} ds^t 
% \end{split}
% \end{equation}
% 
% The first non endpoint
% 
% \begin{equation}
% \begin{split}
% \overbrace{m_{f_{s^t}\rightarrow s^{t-1}}(s^{t-1})}^{\text{Back prior $t-1$}} &= \int \overbrace{\N(s^{t}|s^{t-1},\tau^2)}^{f_{s^t}} \overbrace{\N(s^t|\mu_{\text{like}^t},\sigma_{\text{like}^t}^2)}^{\text{Last likelihood $t$}} \overbrace{\N(s^{t}|\mu_{\text{like}^{t+1}},\sigma_{\text{like}^{t+1}}^2 + \tau^2)}^{\text{First back prior}} ds^t \\
% & = \N(s^{t-1}|\mu_{\text{like}^t},\sigma_{\text{like}^t}^2 + \tau^2) \int \N(s^t | \mu_{\times}, \sigma_{\times}^2)\N(s^{t}|\mu_{\text{like}^{t+1}},\sigma_{\text{like}^{t+1}}^2 + \tau^2) ds^t \\
% & \propto  \N(s^{t-1}|\mu_{\text{like}^t},\sigma_{\text{like}^t}^2 + \tau^2) \N(\underbrace{\mu_{\times}}_{\mu_{\times}(s^{t-1})}|\mu_{\text{like}^{t+1}},\sigma_{\text{like}^{t+1}}^2 + \sigma_{\text{like}^{t+1}}^2 + \tau^2) 
% \end{split}
% \end{equation}
...
\paragraph{Update again} When a game outcome comes up for update again, the new downward message $m_{{f_p}\rightarrow p}(p)$ can be calculated by
