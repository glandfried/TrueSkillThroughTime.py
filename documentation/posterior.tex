...
The update for a game outcome $y^t_i(k)$ is performed by the same way as original TrueSkill but saving the upward message from the $k$ game to the individual $i$ shared skill variable $s_i^t$, $m_{f_{p_i^t(k)} \rightarrow s_i^t}(s_i^t)$.
So when a game outcome comes up for update again, the new downward message can be computed as, 
\begin{equation}
 m_{f_{p_i^t(k)} \rightarrow s_i^t}(p_i^t(k)) = \int f_{p_i^t(k)} \frac{p(s_i^t)}{m_{f_{p_i^t(k)} \rightarrow s_i^t}(s_i^t)} ds_i^t
\end{equation}
...
By the sum-product algorithm we know that,
...
\begin{itemize}
 \item \textbf{Prior (t): } $m_{f_{s^t} \rightarrow s^t}(s^t)$
 \item \textbf{Inverse prior (t): } $m_{f_{s^{t+1}} \rightarrow s^t}(s^t)$
 \item \textbf{Within prior (tk): } $p(s_i^t) \cdot m_{f_{p_i^t(k)} \rightarrow s_i^t}(s_i^t)^{-1}$
\end{itemize}
